...
.defines(function() {
  ...
  var TouchPoint = function( x, y, id, state ) {
		if(compVers==1){
			this.x = x/pR*100-posLeft/2;
			this.y = y/pR*100-posLeft/2;
		}
		else{
			this.x = x;
			this.y = y;
	...
	ig.Input.inject({
		...
		touchEvent: function( e ) {
			...
			if(compVers==2){
				for ( var i = e.changedTouches.length; i--; ) {
					...
					this[ 'multi' + e.type ](t.clientX/pR*100-posLeft/2,t.clientY/pR*100-posTop/2,t.identifier);
				}	
			}
			else{
				for ( var i = e.changedTouches.length; i--; ) {		//codigo multitouch cocoon viejo 
					var t = e.changedTouches[ i ];
		...
		multitouchstart: function( x, y, id ) {
			...
			this.touchesInterval[id] = {touch:this.touches[id],interval:new Date()};
		},
		multitouchmove: function( x, y, id ){
			...
			if (this.touchesInterval[id]){
				this.touchesInterval[id].interval = new Date();
			}
		},
		multitouchend: function( x, y, id ) {
			if ( this.touches[ id ] ){
				...
				delete ig.input.touchesInterval[id];
			}
		},
		...
		checkTouches: function(){
			for ( var t in ig.input.touchesInterval ) {//fix multi-touch android browser
				var touchVar = ig.input.touchesInterval[t];
				var touchInterval = new Date(touchVar.interval);
				touchInterval = new Date(touchInterval.setSeconds(touchInterval.getSeconds()+5));
				if(touchInterval<new Date()){//el segundo es una cuestion de performance, nada más
					this.multitouchend(touchVar.touch.x,touchVar.touch.y,touchVar.touch.id);
					if(stickLeft!='undefined'){//esto es un hardcodeo violento, pero despues lo vemos
						if(stickLeft._touchId==touchVar.touch.id){
							stickLeft.reset();
						}
					}
				}
