...
ig.Entity = ig.Class.extend({
	...
	//metodos propios (replicar en nuevas versiones)
	touchesY: function( other ) {
		return ((!(this.pos.y>=other.pos.y+other.size.y||this.pos.y+this.size.y<=other.pos.y))&&(this.pos.y>other.pos.y));
	},
	distanceToY: function( other ) {
		var yd = (this.pos.y + this.size.y/2) - (other.pos.y + other.size.y/2);
		return Math.sqrt(yd*yd );
	},
	distanceToX: function( other ) {
		var xd = (this.pos.x + this.size.x/2) - (other.pos.x + other.size.x/2); 
		return Math.sqrt( xd*xd);
	},
	compensadorTick: function(par){
		return(par*ig.system.fps/((1 / ig.system.tick).round()));
	},
});
