...
ig.BackgroundMap = ig.Map.extend({	
	...
	draw: function() {
		...
		if( this.preRender ) {
			...
			if(!ig.global.wm && ig.game instanceof MyGame){// se agrego esto para que no dibuje el bg en el intranivel
				if(this.name=='background'){
					if(ig.game.cameraPassage.drawBG)this.drawTiled();
				}else this.drawTiled();
			}else{
				this.drawTiled();
			}
	...
	drawTiled: function() {	
		var tile = 0,
			...
			//pxMaxX = ig.system.width + this.tilesize - pxOffsetX, //se comento esto y se reemplazo por lo de abajo
