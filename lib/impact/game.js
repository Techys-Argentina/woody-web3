...
ig.Game = ig.Class.extend({
	...
	removeEntity: function( ent ) {
		...
		if (ent.animSheet.image.dispose)//mod para optimizar memoria (Cocoon)
			ent.animSheet.image.dispose();
	...
	updateEntities: function() {
		for( var i = 0; i < this.entities.length; i++ ) {
			var ent = this.entities[i];
			if( !ent._killed ) {
				if (ent.updateOffScreen||(ent.pos.x>ig.game.screen.x-ent.size.x-300&&ent.pos.x<ig.game.screen.x+ig.system.width+300&&ent.pos.y>ig.game.screen.y-ent.size.y-200&&ent.pos.y<ig.game.screen.y+ig.system.height+200)||!onLevel){
					ent.update();
				}
			}
		}
	...
	checkEntities: function() {
		...
		for( var e = 0; e < this.entities.length; e++ ) {
			...
			if (entity.updateOffScreen||(entity.pos.x>ig.game.screen.x-entity.size.x-300&&entity.pos.x<ig.game.screen.x+ig.system.width+300&&entity.pos.y>ig.game.screen.y-entity.size.y-200&&entity.pos.y<ig.game.screen.y+ig.system.height+200)||!onLevel){
