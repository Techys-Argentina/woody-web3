...
.defines(function() {
	...
	ig.ScreenFader = ig.Class.extend({
		...
		drawImageTiledOnScreen: function() {
			...
			if(this.options.centrado){
				this._anim.draw(ig.system.width/2-this.options.tileWidth/2, ig.system.height/2-this.options.tileHeight/2); //para centrar la imagen
			}else{
				...
				while (tileY < totalHeight) {
					...
					while (tileX < totalWidth) {// repite la imagen
